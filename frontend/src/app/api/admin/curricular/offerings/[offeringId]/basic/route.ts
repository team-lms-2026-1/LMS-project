...
export async function GET(req: Request, { params }: {params: { offeringId: string }}) {
  return proxyToBackend(req, `/api/v1/admin/curricular-offerings/${params.offeringId}`, {
    method: "GET",
    cache: "no-store"
  });
}
...
export async function PATCH(req:Request, { params }: {params: {offeringId: string }}) {
    ...
    const res = await proxyToBackend(req, `/api/v1/admin/curricular-offerings/${params.offeringId}/basic`, {
        method: "PATCH",
        body,
        cache: "no-store"
    })
