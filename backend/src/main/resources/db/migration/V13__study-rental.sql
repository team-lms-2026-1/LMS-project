...
-- =======================================================
-- 3. Study Room Image (스터디 룸 이미지)
-- =======================================================
CREATE TABLE study_room_image (
    ...
    -- Foreign Key
    CONSTRAINT fk_room_image_room FOREIGN KEY (room_id) REFERENCES study_room(room_id)
);

CREATE INDEX idx_room_image_order ON study_room_image(room_id, sort_order);
...
COMMENT ON TABLE study_room_rental IS 'Prevent time overlap only for APPROVED rentals (Application Logic required)';
