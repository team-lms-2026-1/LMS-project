...
VALUES
  ('EXTRA_CURRICULAR_READ',   '비교과 조회', now(), now()),
  ('EXTRA_CURRICULAR_MANAGE', '비교과 관리', now(), now()),
  ('EXTRA_CURRICULAR_CLASS', '비교과 수강관리', now(), now())
...
FROM auth_role r
JOIN auth_permission p ON p.code IN ('EXTRA_CURRICULAR_READ', 'EXTRA_CURRICULAR_MANAGE', 'EXTRA_CURRICULAR_CLASS')
...
SELECT r.role_id, p.permission_id, now(), 1
FROM auth_role r
JOIN auth_permission p ON p.code IN ('EXTRA_CURRICULAR_READ', 'EXTRA_CURRICULAR_CLASS')
...
SELECT r.role_id, p.permission_id, now(), 1
FROM auth_role r
JOIN auth_permission p ON p.code IN ('EXTRA_CURRICULAR_READ')
