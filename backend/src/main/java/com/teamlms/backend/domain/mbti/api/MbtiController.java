...
public class MbtiController {
    ...
    public ResponseEntity<List<MbtiQuestionResponse>> getQuestions() {
    ...
    @PostMapping("/submit")
    public ResponseEntity<MbtiResultResponse> submitMbti(
        ...
        return ResponseEntity.ok(MbtiResultResponse.from(resultDto));
    }

    @Operation(summary = "최근 결과 조회", description = "사용자의 최근 MBTI 결과를 조회합니다.")
    @GetMapping("/result")
    public ResponseEntity<MbtiResultResponse> getLatestResult(@AuthenticationPrincipal AuthUser authUser) {
        MbtiResultDto resultDto = mbtiService.getLatestResult(authUser.getAccountId());
        return ResponseEntity.ok(resultDto != null ? MbtiResultResponse.from(resultDto) : null);
    }
}
