...
import com.teamlms.backend.domain.extracurricular.repository.ExtraCurricularCompetencyMapRepository;
...
public class CompetencySummaryService {
        ...
        private final ExtraCurricularCompetencyMapRepository extraCompetencyMapRepository;
                ...
                for (Competency comp : competencies) {
                                        ...
                                        for (DiagnosisAnswer ans : answers) {
                                                ...
                                                if (weight > 0 && ans.getScaleValue() != null) {
                                                        BigDecimal score = BigDecimal.valueOf(
                                                                        ans.getScaleValue() * (long) weight);
                                                        if (q.getDomain() == DiagnosisQuestionDomain.SKILL)
                                ...
                                if (e.getRawScore() != null) {
                                        ...
                                        if (weightMap.isPresent()) {
                                                curricular = curricular
                                                                .add(BigDecimal.valueOf((long) e.getRawScore()
                        ...
                        for (var app : applications) {
                                ...
                                ExtraCurricularOffering offering = extraOfferingRepository
                                                .findById(app.getExtraOfferingId()).orElse(null);
                                if (offering == null)
                                        continue;

                                var weightMap = extraCompetencyMapRepository
                                                .findByIdExtraCurricularIdAndIdCompetencyId(
                                                                offering.getExtraCurricularId(),
                                                                comp.getCompetencyId());
                                ...
                                if (weightMap.isPresent()) {
                                        long totalPoints = completionList.stream()
                                                        ...
                                                        .sum();
                                        extra = extra.add(BigDecimal
                                                        .valueOf(totalPoints * weightMap.get().getWeight()));
                                }
                        }

                        // 4. 업데이트
                        updateStudentCompetencySummary(semesterId, studentAccountId, comp.getCompetencyId(),
                                        diagSkill, diagAptitude, curricular, extra, BigDecimal.ZERO);
                }
        }
        ...
        public void recalculateAllSummaries(Long semesterId) {
                ...
                for (com.teamlms.backend.domain.account.entity.StudentProfile student : students) {
                        ...
                        for (Competency comp : competencies) {
                                ...
                                // 4. 업데이트
                                updateStudentCompetencySummary(semesterId, studentId, comp.getCompetencyId(),
                                                diagSkill, diagAptitude, curricular, extra, BigDecimal.ZERO);
                        }
                }
        ...
        public void calculateCohortStatistics(Long semesterId) {
                ...
                for (Competency comp : competencies) {
                        ...
                        int totalTarget = (int) studentProfileRepository.count(); // 등록된 모든 학생 대상
